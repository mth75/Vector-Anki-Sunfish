...
def main():
    ...
    with anki_vector.Robot(args.serial) as robot:
        ...
        robot.screen.set_screen_to_color(anki_vector.color.off, duration_sec=2.0)
...
# Argument for connection to our beloved little one
args = anki_vector.util.parse_command_args()
...
def make_text_image(text_to_draw, x, y):
    dimensions = (184, 96)
    text_image = Image.new('RGBA', dimensions, (0, 0, 0, 255))
    font = ImageFont.truetype('Roboto-Bold.ttf', size=45)
    dc = ImageDraw.Draw(text_image)
    dc.text((x, y), text_to_draw, font=font, fill=(255, 255, 255, 255))
    return text_image
        ...
        while True:
            ...
            while move not in pos.gen_moves():
                ...
                if match:
                    ...
                    robot.say_text("Please enter a move like g8f6")
            ...
            if score == MATE_UPPER:
                robot.say_text("Checkmate!")
                print("Checkmate!")

            # The black player moves from a rotated position, so we have to
            # 'back rotate' the move before printing it.
            my_move = 'My move' + '...' + render(119-move[0]) + '...' + render(119-move[1])
            robot.say_text(my_move)
            print("My move:", render(119-move[0]) + render(119-move[1]))
            pos = pos.move(move)
            face_image = make_text_image(render(119-move[0]) + render(119-move[1]), 42, 12)

            # Convert the image to the format used by the Screen
            screen_data = anki_vector.screen.convert_image_to_screen_data(face_image)
            robot.screen.set_screen_to_color(anki_vector.color.off, duration_sec=2.0)
            robot.screen.set_screen_with_image_data(screen_data, 4.0, interrupt_running=True)
